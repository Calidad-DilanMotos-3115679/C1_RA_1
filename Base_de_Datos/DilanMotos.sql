...
-- 4. Tabla moto (ACTUALIZADA: Se agregó el campo placa UNIQUE y NOT NULL)
CREATE TABLE moto (
...
CREATE TABLE pqrs (
    ...
    CONSTRAINT CHK_PQRS_Comentario CHECK (char_length(comentario_usuario) >= 6),
    CONSTRAINT CHK_PQRS_Descripcion CHECK (char_length(descripcion) >= 6),
    CONSTRAINT CHK_PQRS_Asunto CHECK (char_length(asunto) >= 6),
    FOREIGN KEY (id_usuario) REFERENCES usuario(id_usuario)
) ENGINE=InnoDB;
-- 14. Tabla cotizacion
